...
```
recipe "reports": `url`: "fake-secret" is a forbidden path — contact the administrator
```

It does not say which of the three set it, and the built-in names above refuse with the
same sentence. Whoever hits this cannot act on either, and naming the config key mostly
invites editing it out.
